        ...
        IProvidesRegisterCollection<ByteRegisterCollection>
    {
        ...
        private void InitializeRegisters()
        {
            ...
            Registers.BlockSizeCount.Define32(this)
                ...
                // Bits [14:12] are SDMA Buffer Boundary per the SDHCI spec, not reserved -
                // only bit 15 actually is. This model always transfers the whole
                // block/multi-block in one go (no SDMA-boundary-triggered interrupts), so
                // the field is accepted and stored but otherwise unused.
                .WithValueField(12, 3, name: "SDMA_BUF_BDARY")
            ...
            Registers.ClockTimeoutSoftwareReset.Define32(this)
                ...
                // Real field (SDHCI spec), not unimplemented - this model has no
                // line-timing to enforce a data timeout against, so accept and store
                // the value without acting on it, same rationale as SDMA_BUF_BDARY above.
                .WithValueField(16, 4, name: "DATA_TIMEOUT_COUNTER")
